...
public class SCIM2ConnectorServiceComponent {
    ...
    protected void activate(ComponentContext context) {
        ...
        try {
            SCIM2ProvisioningConnectorFactory scim2ProvisioningConnectorFactory = new SCIM2ProvisioningConnectorFactory();
            context.getBundleContext().registerService(AbstractProvisioningConnectorFactory.class.getName(), scim2ProvisioningConnectorFactory, null);
            if (log.isDebugEnabled()) {
                log.debug("SCIM 2.0 Provisioning Connector bundle is activated");
            }
        } catch (Throwable e) {
            log.error(" Error while activating SCIM 2.0 Provisioning Connector ", e);
        }
    }
}
